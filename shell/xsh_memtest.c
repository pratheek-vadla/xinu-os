...
/* TEST 3: Malloc all in one block */ 
void test3(void) {
  pid32 pid = getpid();
  uint32 size = heaptab[pid].size;
  char* block = heaptab[pid].startaddr;
...
/* TEST 4: Malloc all in small blocks */ 
void test4(void) {
  pid32 pid = getpid();
  uint32 size = heaptab[pid].size;
  char* block = heaptab[pid].startaddr;
...
/* TEST 5: Overallocate in one block */ 
void test5(void) {
...
/* TEST 6: Overallocate in many blocks */ 
void test6(void) {
...
/* TEST 8: Free multiple and coalesce */ 
void test8(void) {
  pid32 pid = getpid();
  uint32 size = heaptab[pid].size;
  char* block = heaptab[pid].startaddr;
